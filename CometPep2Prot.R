...
if (todo == 0 || todo == 1) {
	...
	for(i in 1:length(targetCometFiles)) {
		...
		if (dim(cosmicResult)[2] == 19) {
			...
			y <- paste('java -XX:ParallelGCThreads=4 -jar /mnt/beegfs/agarin/bin/PeptideMatchCMD_1.0.jar -a query -i ', currentFolder, '/PeptideMatch_', dbName, '_index -Q ',targetOutCometFiles[i],' -l -e -o ', targetPMOutCometFiles[i], ' > ', targetLogCometFiles[i], sep = "")
			system(y)
			y <- NULL
...
if (todo == 0 || todo == 2) {
	...
	for(i in 1:length(decoyCometFiles)) {
		...
		if (dim(cosmicResult)[2] == 19) {
			...
			y <- paste('java -XX:ParallelGCThreads=4 -jar /mnt/beegfs/agarin/bin/PeptideMatchCMD_1.0.jar -a query -i ', currentFolder, '/PeptideMatch_', dbName, '_', decoy_id,'_index -Q ',decoyOutCometFiles[i],' -l -e -o ',decoyPMOutCometFiles[i], ' > ', decoyLogCometFiles[i], sep = "")
			system(y)
			y <- NULL
