...
nextprotfolder <- args[3]	# /mnt/beegfs/agarin/dato-activo/03_Analysis/agarin/23_neXtprot_20170801_Nov2017/
...
results_comet_NX <- results_comet_NX[!(is.na(results_comet_NX$NextprotID)),]
results_mascot_NX <- results_mascot_NX[!(is.na(results_mascot_NX$NextprotID)),]
results_omssa_NX <- results_omssa_NX[!(is.na(results_omssa_NX$NextprotID)),]
results_tandem_NX <- results_tandem_NX[!(is.na(results_tandem_NX$NextprotID)),]
...
compare4List(paste(results_comet_NX$NextprotID), paste(results_mascot_NX$NextprotID), paste(results_omssa_NX$NextprotID), paste(results_tandem_NX$NextprotID), "Comet results", "Mascot results", "Omssa results", "Tandem results",  "Proteins (Nextprot) for each search engine")
dev.off()
...
results_comet_NX_annot[paste(results_comet_NX_annot$PeptideSeq) %in% paste(nextProtDB20170801_peptidesXProtAll_filterAA_disc$Peptide), "discriminant"] <- 1
...
results_mascot_NX_annot[paste(results_mascot_NX_annot$PeptideSeq) %in% paste(nextProtDB20170801_peptidesXProtAll_filterAA_disc$Peptide), "discriminant"] <- 1
...
results_omssa_NX_annot[paste(results_omssa_NX_annot$PeptideSeq) %in% paste(nextProtDB20170801_peptidesXProtAll_filterAA_disc$Peptide), "discriminant"] <- 1
...
results_tandem_NX_annot[paste(results_tandem_NX_annot$PeptideSeq) %in% paste(nextProtDB20170801_peptidesXProtAll_filterAA_disc$Peptide), "discriminant"] <- 1
